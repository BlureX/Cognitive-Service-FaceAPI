var currentImage : HTMLInputElement = <HTMLInputElement>  $("#image-file")[0];
...
function readPicture(input):void {
    if (input.files) {
        var image_reader :FileReader= new FileReader();
...
function processImage(callback) :void {
    var image : any = currentImage.files[0];
